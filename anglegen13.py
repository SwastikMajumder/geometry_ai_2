...
def compute(points, point_pairs, command_given, perpendicular_angle_list):
    list_1 = solvemath15.EqList()
    list_2 = solvemath15.EqList()
    eq_list_3 = []
    def n2a(number):
            ...
            if "congruent" in item:
                ...
                for item2 in itertools.combinations(m2, 2):
                    ...
                    if line1 == line2:
                        continue
                    tmp = {}
                    tmp[line_sort(line1)] = 1
                    tmp[line_sort(line2)] = -1
                    list_2.add_equation(tmp)
    ...
    def revisoceles():
        for item in list_1.pair_eq:
            for tri in all_tri:
                if all(
                    standard_angle(x) in item for x in itertools.permutations(tri, 3)
                ):
                    angle1 = item[0]
                    angle2 = item[1]
                    tmp = {}
